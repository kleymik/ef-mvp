...
function prntArray(arr)    // print array assuming it's row-major
{
    for(r=0; r<arr.length; r++){
        console.log(arr[r]);            //for(j=0; j<assetCount; j++){ corrMtx[i][j] = inputCorrMtx[i][j]; }
    }
}
...
function calcPfRiskFromPfRtns(mAsstCorr, vAsstRtns, vAsstVols, rtnSamplePoints)  {   // for testing
    ...
    foo = rtnSamplePoints.map(function(mu) { return { "rsk": 300 - 30 * mu,
                                                      "rtn": 100 * Math.sqrt(Math.abs(d + Math.pow(mu * c - a, 2) / (c * d)))} });
    return foo;
}
    // hence:
    // where:
