...
function prntArray(arrNm) {
    // print array assuming it's row-major    
    console.log("----" + arrNm + "----");
    arr = eval(arrNm);
    for(r=0; r<arr.length; r++){ console.log(arr[r]);}
}
...
function mtxCondNum(mtx) {
    // calculate matrix condition number
    // to monitor getting close to sigularity
    singVals = numeric.svd(mtx).S;
    return singVals[0]/singVals[singVals.length-1];
}

function calcPfRiskEnRtn2(mAsstCorr, vAsstVols, vAsstRtns, vAsstWghts, numAssts) {
    // for testing
    console.log("called calcPfRiskEnRtn");
    return { rsk:55, rtn:11 };
}

function calcPfRiskFromPfRtns(mAsstCorr, vAsstRtns, vAsstVols, rtnSamplePoints) {    
    ...
    console.log("calcPfRiskFromPfRtns:Covar Condition Number:"+mtxCondNum(mAsstCov));

    a = numeric.dot(vOnes,     numeric.dot(mAsstCovInv, vAsstRtns));            // see notebook: a = ones.T @ cov**(-1) @ mu
    b = numeric.dot(vAsstRtns, numeric.dot(mAsstCovInv, vAsstRtns));            // see notebook: b = mu.T   @ cov**(-1) @ mu
    c = numeric.dot(vOnes,     numeric.dot(mAsstCovInv, vOnes));                // see notebook: c = ones.T @ cov**(-1) @ ones
    d = b * c - Math.pow(a,2);                                                  // see notebook: d = b * c - a**2
...
function calcPfRiskEnRtn(mAsstCorr, vAsstVols, vAsstRtns, vAsstWghts) {
    // calculate Portfolio's Effective Risk and Total Return
